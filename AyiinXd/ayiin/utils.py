...
async def autopilot():
    ...
    try:
        r = await bot(
            CreateChannelRequest(
                title="ɴɪɢʜᴛ ᴜꜱᴇʀʙᴏᴛ ʟᴏɢs",
                about="» Group log Created by: Night-Userbot\n\n» UPDATES: @night_userbot",
                megagroup=True,
            ),
        )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://graph.org/file/714e51c0417a6c83e686d-492e000414e75365a2.jpg", "photoyins.jpg"
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"Night{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"Night{str(who.id)[6:]}{ran}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
